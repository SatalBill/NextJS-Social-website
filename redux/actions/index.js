...
export function fetchEvents() {
  if (!getLocalStorage("events")) {
    setLocalStorage("events", JSON.stringify(eventData)); // If storage is empty set demo data
  }
  let events = JSON.parse(getLocalStorage("events")); //Get data from Storage
  return {
    type: FETCH_EVENTS,
    payload: events,
  };
...
export function createEvent(values) {
  let events = JSON.parse(getLocalStorage("events"));
  events.push(values); //Push New Item
  setLocalStorage("events", JSON.stringify(events)); //Update Storage
  return {
    type: CREATE_EVENT,
    payload: events,
  };
