...
const ChatList = (props) => {
    ...
    return (
        <div className='chatlist' key={chats.length}>
            {
                chats.map((chat, index) => {
                    showid = chat.userid !== before_userid && chat.userid !== 'me';
                    show_timestring = showid || timeToLocal(chat.time) !== before_timestring;
                    before_userid = chat.userid;
                    before_timestring = timeToLocal(chat.time);
                    return (
                        <div key={ index } className='chatitemparent' style={{textAlign: chat.align}}>
                            <table width='100%' >
                                <tbody >
                                    <tr >
