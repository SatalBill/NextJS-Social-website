...
class FriendList extends Component {
	...
	checkIsOnline = (userId) => {
		const online = this.props.onlineUsers.find(
			...
			<div className="fr-list-body pt-3">
				{this.state.chatRoomList &&
					...
					this.state.chatRoomList.map((f_data, k) => {
						const user =
							f_data.user1.id !== this.props.myProfile.id
								? f_data.user1
								: f_data.user2;
						...
						return (
							<div
								...
								key={k}
								onClick={() => this.joinFriendRoom(user, f_data.unreadMessages)}
							>
